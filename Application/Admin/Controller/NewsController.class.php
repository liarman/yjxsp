...
class NewsController extends AdminBaseController{
    ...
    public function ajaxNewsList(){
        ...
        $data=D('News')->limit($offset.','.$rows)->select();
        $result["rows"] = $data;
    ...
    public function ajaxNewsAll(){
        $townid=I("get.townid");
        $data=D('News')->where(array('townid'=>$townid))->select();
        $this->ajaxReturn($data,'JSON');
    }
    public function ajaxBoundary(){
        $vilage_id=I("get.village_id");
        $data=D('NewsBoundary')->where(array('villageid'=>$vilage_id))->select();
        $this->ajaxReturn($data,'JSON');
    }
    public function addNewsBoundary(){
        if(IS_POST){
            $data['longitude']=I('post.longitude');
            $data['latitude']=I('post.latitude');
            $data['villageid']=I('post.villageid');
            unset($data['id']);
            $result=D('NewsBoundary')->addData($data);
            if($result){
                $message['status']=1;
                $message['message']='保存成功';
            }else {
    ...
    public function editNewsBoundary(){
        if(IS_POST){
            $data['id']=I('post.id');
            $data['longitude']=I('post.longitude');
            $data['latitude']=I('post.latitude');
            $data['villageid']=I('post.villageid');
            $where['id']=$data['id'];
            $result=D('NewsBoundary')->editData($where,$data);
            if($result){
                $message['status']=1;
                $message['message']='保存成功';
            }else {
     ...
     * 删除
     */
    public function deleteNewsBoundary(){
        $id=I('get.id');
        $map=array(
            'id'=>$id
        );
        $result=D('NewsBoundary')->deleteData($map);
        if($result){
            $message['status']=1;
            $message['message']='删除成功';
        }else {
     ...
     * ajaxNewsAccountList
     */
    public function ajaxNewsAccountList(){
        $townid=I('get.townid');
        $meetid=I('get.meetid');
